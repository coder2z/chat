...
$(".select").change(function(){
	...
	if(option == '禁用'){
					$.ajax({
			type: 'post',
			url: "../blacklists/state/"+$('#companyId').val(),
			dataType: "json",
			data:{
				state:0
			},
		...
		$.ajax({
			...
			success: function(data) {
				if(data.code == 200) {
					...
					let information = JSON.parse(data.data[0].evidence).credential;
					$(".black-main-two #user-name").val(name);
					$(".black-main-two #name").val(name);
					$(".black-main-two #phone").val(phone);
					$(".black-main-two .jubaolan-right textarea").val(information);
