...
$(function () {
    ...
    $("#sendButton").click(function() {
        $('.chatContent_div').animate({scrollTop:10000000000000},0);
    });
    //图片放大
            ...
            switch (Data.code) {
                case 200:
                    ...
                    $(".serviceName_content").click(function () {
                        ...
                        $('.chatContent_div').animate({scrollTop: 10000000000000}, 500);
                    })
            }
        },
        complete: function () {
    ...
    var serName = $(".welcome").find("span").attr('alt');
    var ws = new WebSocket("ws://"+document.domain+":8282");
    ws.onopen = function (e) {
        $('#sendButton').click(function () {
            let inputValue = $(".chatInput_input").html().replace(/&nbsp;/g, "");
            if (inputValue == '') {
                alert('不为空！');
                return;
            }
            var userId = $(".serviceName_active").find('div').find('img').attr('alt');
            var userName = $(".serviceName_active").find('div').eq(1).find('a').html();
            var str = inputValue.replace(/(^\s*)|(\s*$)/g, '');//去除空格;
            str = str.replace(/"/g, "'");
            if (str != "") {
                var message = '{"data":"' + str + '","type":"say","fromId":"' + serId + '","toId":"' + userId + '","fromName":"' + serName + '","toName":"' + userName + '"}';
        ...
        ws.onmessage = function (e) {
            ...
            switch (message.type) {
                ...
                case "text":
                    ...
                    if (active) {
                        let temp = active.find('div').find('img').attr('alt');
                        if (temp == message.fromId) {
                            $('#h_chat_content').append(`<div class="service_person"><div class="service_person_hand">
...
function bindActive() {
    $(".serviceName .serviceName_content").click(function () {
        nowUserId = $(this).find('div').find('img').attr('alt');
        nowSerId = $(".welcome").attr('id');
        var userId = $(this).find('div').find('img').attr('alt');
        let noRead = $(this).find("div").eq(2);
        $(this).addClass("serviceName_active").siblings().removeClass("serviceName_active");
        if (noRead) {noRead.remove()}
        $.ajax({
            type: 'get',
                    ...
                    case 200:
                        ...
                        for (var i = 0; i < length; i++) {
                            if (useData[i].fromid == userId) {
                                ...
                                $('#h_chat_content').append(`<div class="robot"><div class="robot_hand">
                                    <div class="robot_hand_pic"></div>
            ...
            $.ajax({
                ...
                success: function (Data) {
                    ...
                    for (let i = 0; i < noActive.length; i++) {
                        if (noActive.eq(i).find('div').find('img').attr('alt') == messageId) {
                            noActive.eq(i).find("div").eq(2).remove();
...
$(document).on("change","#file-input1",function (e) {
  ...
  $(document).on("click",'.report_button button',function () {  
    ...
    if($('.add>img').attr('src') != '' && $('.report_input textarea').val() != ''){
    $.ajax({
        type: "post",
